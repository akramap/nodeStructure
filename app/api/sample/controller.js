 ...
 * Get Team matches
 * @property {string} params.id - Team Id.
 * @property {array} query.tags - Array of match tags.
 * @returns {Object}
 */
async function getTeamMatches(params, query) {
  // Validating param
  const validParam = await validator.getTeamMatches.validate({ params });

  const { id } = validParam.params;

  // Validating param
  const validQuery = await validator.getTeamMatches.validate({ query });

  // Getting team details
  const existingDoc = await service.findMatches({
    id,
...
export default { get, list, create, update, remove, getTeamMatches };
